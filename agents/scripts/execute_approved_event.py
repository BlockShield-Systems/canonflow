...
def claim_execution(
    ...
    claim_path = state_dir / f"{proposal_sha256}.claimed.json"
    claim = {
        "schema_version": "1.0",
        "claim_type": "canonflow_controlled_event_execution_claim",
        "proposal_sha256": proposal_sha256,
        "claimed_at_utc": datetime.now(timezone.utc).isoformat(),
        "execution_scope": "single_exact_retry",
    ...
    try:
        ...
        raise RuntimeError(
            "Execution claim already exists for proposal SHA-256: "
            f"{proposal_sha256}"
        ) from error
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Persistent private directory containing atomic "
    ...
    try:
        ...
        if args.execution_state_dir is None:
            raise RuntimeError(
                "--execution-state-dir is required with --execute."
            )

        execution_claim = claim_execution(
            state_dir=args.execution_state_dir,
        ...
        evidence = {
            ...
            "execution_claim": str(execution_claim),
        }
